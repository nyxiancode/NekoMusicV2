#
# Copyright (C) 2021-2022 by Neko_Help @ Github, < https://github.com/Jankarikiduniya >.
# A Powerful Music Bot Property Of Rocks Indian Largest Chatting Group
# All rights reserved. © Alisha © Neko © Yukki © AnonXMusic
...
async def gen_thumb(videoid, user_id, theme):
    ...
    try:
        ...
        try:
            text_w, text_h = draw.textsize(f"NEKO MUSIC IS PLAYING OP", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"NEKO MUSIC IS PLAYING OP",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
                f"Neko Music One Of The Most Advanced Telegram Music Bot", font=arial
            )
            draw.text(
                ((1280 - text_w) / 2, 80),
                f"Neko Music One Of The Most Advanced Telegram Music Bot",
                fill="green",
                font=arial,
            )
            if para[0]:
...
async def gen_qthumb(videoid, user_id, theme):
    ...
    try:
        ...
        para = textwrap.wrap(title, width=32)
        try:
            text_w, text_h = draw.textsize(f"NEKO ADDED THIS SONG TO QUEUE", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"NEKO ADDED THIS SONG TO QUEUE",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
