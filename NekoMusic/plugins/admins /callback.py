...
from NekoMusic import YouTube, app
from NekoMusic.core.call import Neko
from NekoMusic.misc import SUDOERS, db
from NekoMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from NekoMusic.utils.decorators.language import languageCB
from NekoMusic.utils.formatters import seconds_to_min
from NekoMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from NekoMusic.utils.stream.autoclear import auto_clean
from NekoMusic.utils.thumbnails import gen_thumb
from NekoMusic.utils.theme import check_theme
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Neko.pause_stream(chat_id)
        ...
        await Neko.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention), disable_web_page_preview=True
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Neko.stop_stream(chat_id)
        ...
        await Neko.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Neko.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Neko.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Neko.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Neko.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            theme = await check_theme(chat_id)
            ...
            try:
                await Neko.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Neko.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
